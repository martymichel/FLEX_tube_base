"""
Settings Dialog - Vollständiger Einstellungsdialog 
Alle Dialog-Fenster der Anwendung mit Tab-basiertem Layout, erweiterte Klassenzuteilung und Referenzlinien
GEFIXT: Modbus-Controller-Reset und andere kritische Funktionen wiederhergestellt
"""
...
class SettingsDialog(QDialog):
    ...
    # =============================================================================
    # MODBUS-AKTIONS-HANDLER - GEFIXT: ORIGINALE FUNKTIONEN WIEDERHERGESTELLT
    # =============================================================================
    
    def handle_modbus_reset(self):
        """GEFIXT: WAGO Controller Reset - ORIGINAL-FUNKTION aus alter dialogs.py"""
        # Prüfe Admin-Rechte über parent_app Kette
        if not (hasattr(self.parent_app, 'app') and 
                hasattr(self.parent_app.app, 'user_manager') and 
                self.parent_app.app.user_manager.is_admin()):
            QMessageBox.warning(
                self,
                "Zugriff verweigert",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # GEFIXT: Originale restart_controller Methode aufrufen
                modbus_manager = self.parent_app.app.modbus_manager
                if modbus_manager.restart_controller():
                    ...
                    import time
                    time.sleep(2)  # Kurz warten nach Reset
    ...
    def handle_modbus_reconnect(self):
        """GEFIXT: Modbus Neuverbindung - ORIGINAL-FUNKTION aus alter dialogs.py"""
        # Prüfe Admin-Rechte über parent_app Kette
        if not (hasattr(self.parent_app, 'app') and 
                hasattr(self.parent_app.app, 'user_manager') and 
                self.parent_app.app.user_manager.is_admin()):
        ...
        try:
            ...
            # GEFIXT: Originale force_reconnect Methode aufrufen
            if modbus_manager.force_reconnect():
                # Services neu starten
                modbus_manager.start_watchdog()
                modbus_manager.start_coil_refresh()
                
                # UI Status aktualisieren
                if hasattr(self.parent_app.app, 'ui'):
                    self.parent_app.app.ui.update_modbus_status(True, modbus_manager.ip_address)
            ...
            logging.error(f"Fehler bei Modbus-Neuverbindung: {e}")
            QMessageBox.critical(
                self,
                "Fehler",
    ...
    def update_modbus_connection_status(self, connected):
        ...
        # Buttons nur aktivieren wenn Admin
        is_admin = (hasattr(self.parent_app, 'app') and 
                   hasattr(self.parent_app.app, 'user_manager') and 
                   self.parent_app.app.user_manager.is_admin())
